...
import statementChart from "./statementFitLineChart.md";
import ChartDialogFitLineSettingContainers from '../../redux/containers/ChartDialogFitLineSettingContainers.js';


class AppFitLineCharts extends Component {
  ...
  handleFitLine(xval,yval,earr,displayname,displayconsole) {
    ...
    const chart_optimized_time = this.props.chart_fitline_optimized_time;
    const chart_loss_value = this.props.chart_fitline_loss_value;
    //const chart_leastsq_real_value = this.props.chart_leastsq_real_value;
    this.props.loadingEnableCreate(true);
    let url = global.constants.const_api_url + "/app/cal/fitline?accesstoken=" + accesstoken;
    fetch(url,{
      method: "Post",
      mode: "cors",
      headers: {
      ...
      body: JSON.stringify({
        ...
        yval: yval,
        title: chart_title,
        opttime: chart_optimized_time,
        loss: chart_loss_value
      })
     })
     ...
     .then((dat) => {
         ...
         if(this.props.chart_leastsq_real_value_result_enable) {
            row = {};
            row.context = global.constants.const_app_chart_fitline_loss+": "+data[0].loss; 
            arrlst.push(row);
         }
  ...
  render() {
    ...
    return (
        <div>
              <MyHeader myheadertitle={ global.constants.const_app_chart_fitline } />
              <WingBlank size="lg">
                  <WhiteSpace size="lg" />
                  ...
                  <ReactMarkdown
                    ...
                    escapeHtml={false}
                  />
                  <ChartDialogFitLineSettingContainers />
              </WingBlank>
        </div>
    );
  }
...
const AppFitLineChartsWrapper = createForm()(AppFitLineCharts);
export default AppFitLineChartsWrapper;
