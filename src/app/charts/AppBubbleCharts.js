...
class AppBubbleCharts extends Component {
  ...
  handleClickSubmit (e) {
       ...
       for ( var i=0;i<arr.length;i++) {
           ...
           if(subarr.length>2&&IsNumberFunc(subarr[0].trim())&&IsNumberFunc(subarr[1].trim())&&IsNumberFunc(subarr[2].trim())){
               row = {};
               row.x = Number(subarr[0].trim());
               row.y = Number(subarr[1].trim());
               row.r = Number(subarr[2].trim());
               xarr.push(row);
  ...
  render() {
    ...
    return (
        <div>
              <MyHeader myheadertitle={ global.constants.const_app_chart_bubble } />
              <WingBlank size="lg">
                  <WhiteSpace size="lg" />
...
const AppBubbleChartsWrapper = createForm()(AppBubbleCharts);
export default AppBubbleChartsWrapper;
